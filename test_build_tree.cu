  ...
  for (int s = 0; s < 4; ++s) {
    ...
    if (v == -1) {
      std::printf("%s%s (step %d): empty\n",
                  branch.c_str(), kQuadrantName[s], s);
    } else if (v == -2) {
      ...
      std::printf("%s%s (step %d): body %d @ (%.3f, %.3f)\n",
                  branch.c_str(), kQuadrantName[s], s,
                  v, points[v * 2], points[v * 2 + 1]);
    } else {
      std::printf("%s%s (step %d): cell %d  center=(%.3f, %.3f) r=%.3f\n",
                  branch.c_str(), kQuadrantName[s], s,
                  v, ccx, ccy, child_r);
      print_subtree(cells, points, number_of_points,
                    v, ccx, ccy, child_r, sub_prefix);
    }
  }
}
...
void collect_bodies(const std::vector<int>& cells,
                    int number_of_points,
                    int cell_idx,
...
bool run_case(const char* label,
  ...
  print_tree(out_cells, host_points, number_of_points,
  ...
  for (int b : found) {
    if (b < 0 || b >= number_of_points) { reach_ok = false; break; }
    if (++seen[b] > 1) { reach_ok = false; }
  }
...
int main() {
  ...
  const bool ok1 = run_case("level-1: 4 bodies, no subdivision",
                            level1, 0.0f, 0.0f, 2.0f);
